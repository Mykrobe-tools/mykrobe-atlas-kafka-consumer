...
public class CreateSearchTransformer extends SearchTransformer {

	public CreateSearchTransformer(KeyValueStore<SearchKey, Search> stateStore, DebeziumSearchPayload payload) {
		super(stateStore, payload);
	}

	@Override
	public KeyValue<SearchKey, Search> transform() {
		MongoSearch mongoSearch = payload.getAfter();
		SearchKey key = new SearchKey();
		Search search = new Search();
		String id = mongoSearch.getId().getOid();
